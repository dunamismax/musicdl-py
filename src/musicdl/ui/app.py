...
class MusicDownloaderApp(App[None]):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="main-content"):
            ...
            with Horizontal(id="topbar"):
                ...
                yield Input(
                    placeholder="Enter CSV file path or drag & drop",
                    id="csv-input"
                )
    ...
    def on_mount(self) -> None:
        ...
        # Set up TUI logging
        self.tui_log_handler = add_tui_handler(
            self._log_to_ui,
            level=logging.INFO
        )
        ...
        self.downloader = YTDLPDownloader(
            config=self.config,
            progress_callback=self._log_to_ui
        )
    ...
    def action_scan_csv(self) -> None:
        ...
        try:
            ...
            mode = ("Artist/Track columns detected" 
            ...
            self._update_status(f"CSV loaded - {len(self.csv_detection.preview_rows)} rows", "success")
    ...
    def _start_downloads(self) -> None:
        ...
        artist_col = artist_select.value if artist_select.value != Select.BLANK else None
            ...
            try:
                ...
                if result.success:
                    if self.dry_run_mode:
                        self.call_from_thread(self._log_to_ui, f"Found: {item.display_name}")
                    else:
                        self.call_from_thread(self._log_to_ui, f"Downloaded: {item.display_name}")
                else:
                    self.call_from_thread(self._log_to_ui, f"Failed: {item.display_name} - {result.error_message}")
    ...
    def _download_complete(self) -> None:
        ...
        # Calculate results
        success_count = sum(1 for item in self.track_items 
                          if item.status in [TrackStatus.DONE, TrackStatus.FOUND])
        error_count = sum(1 for item in self.track_items 
        ...
        if error_count == 0:
            self._update_status(f"Complete - {success_count} tracks processed", "success")
        else:
            self._update_status(
